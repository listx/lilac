...
#+begin_sidenote
The /lilac/ is a [[https://en.wikipedia.org/wiki/Syringa][small flowering plant]].
#+end_sidenote
...
(Ignore the =js-toc-scrollspy= bit, as that's for the [[* Table of contents]["Table of contents"]]
section below.)
...
,#+HTML_HEAD: <script src="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0/dist/js/bootstrap.bundle.min.js" integrity="sha384-geWF76RCwLtnZ8qwWowPQNguL3RmwHVBC9FhGdlKrxdiJJigb/j/68SIy3Te4Bkz" crossorigin="anonymous"></script>
...
Make the "Table of Contents" float off to the left so that it is always visible.

#+name: __NREF__css-toc
#+begin_src css
#table-of-contents {
  position: sticky;
  top: 0;
...
#text-table-of-contents ul li ul li ul li ul li {
  margin-bottom: 0;
}
#table-of-contents a {
  display: block;
...
We also delete the text "Table of Contents" because it looks too intrusive when
it is placed in the sidebar area.

#+name: __NREF__Makefile-replace-toc-text
#+begin_src makefile
sed -i 's|<h2>Table of Contents</h2>||' README.html
sed -i 's|<body>|<body data-bs-spy="scroll" '\
'data-bs-target="#text-table-of-contents">|' README.html
#+end_src

Lastly we use some JavaScript to automatically update the Table of Contents when
we scroll up or down the page, using Bootstrap's "scrollspy" feature.
