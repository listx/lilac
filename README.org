...
effort to make /tangling/ (extracting source code from [[source code block][source code blocks]]) work
...
**** Run =lilac-publish= on temporary files

Return the full path to the HTML file which was just published.
...
- <<monoblock>> *monoblock* :: an Org mode source code block with a =#+name:
  ...
  with the same name.
- <<Noweb>> *Noweb* :: A literate programming tool from 1989 that still works and from
...
- <<source code block>> *source code block* :: An Org mode facility that allows
  you to enclose a multiline text (typically source code) with =#+begin_src ...=
  and =#+end_src= lines. They are enclosed in a separate background color in the
  HTML output, and are often used for illustrating source code listings. The
  format is =#+begin_src LANGUAGE_NAME= where =LANGUAGE_NAME= is the name of the
  programming language used for the listing. If the name is a recognized name,
  it will get syntax highlighting in the output automatically.
