...
weaving. Typically, you would have a browser pointed to =README.html= and
refresh it after editing the underlying =README.org= file. After every change to
the Org file, you can run =make= to tangle, weave, and run unit tests.
...
We use [[https://github.com/nmattia/niv][=niv=]] to update the dependencies sourced by =shell.nix=. Niv uses two
...
We simply tangle the =README.org= file to get all the code we need.
...
Some code blocks are generated by executing code in other blocks. This way, you
can use all the power of Org mode (as well as any supported programming
language) as a kind of meta-programming system. For example, see [[* Create
=lilac.theme= file]].

Orgmode by default disables automatic evaluation of source code blocks, because
it is a big security risk. For us, we know that we want to allow code execution,
so we disable the evaluation confirmation check. This way, evaluation can still
work even in batch mode.

#+name: __NREF__lilac_dot_el-allow-code-evaluation
...
This is why we provide a [[file:shell.nix][=shell.nix=]] file in this repo.
...
We use JavaScript to make the HTML document more useful. The main ideas are:

1. [[* Self-linked headlines][self-linked headlines]], and
2. current heading tracking in the left sidebar (effectively a Table of
   Contents).

These are included in =lilac.js=, as below.
...
Different destination elements need different selectors. We cover them all here.
...
below (note the trailing parentheses =()=, which evaluates the referenced code
